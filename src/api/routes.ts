...
export function registerRoutes(app: FastifyInstance): void {
  ...
  app.post("/api/import", async (req, reply) => {
    ...
    if (content.length > 1_000_000)
      return reply.code(413).send({ error: "file too large (limit 1 MB)" });

    const filename = typeof body.filename === "string" ? body.filename : "";
    const requested =
      body.protocol && body.protocol !== "auto" ? body.protocol : null;
    const protocol = (requested ??
      detectProtocol(filename, content)) as Protocol | null;
    if (!protocol || !["rest", "socket", "kafka"].includes(protocol)) {
      return reply.code(422).send({
        error:
          "could not tell which runner this script belongs to — pick a protocol and import again",
      });
    }
    ...
    try {
      ...
      if (!parsed.success) {
        return reply
          .code(422)
          .send({ error: parsed.error.issues, warnings: result.warnings });
      }
      return {
        protocol,
  ...
  app.post("/api/runs", async (req, reply) => {
    ...
    const ids = body.profileIds?.length
      ? body.profileIds
      : body.profileId
        ? [body.profileId]
        : [];
    ...
    if (ids.length) {
      for (const id of ids) {
        ...
        if (!p)
          return reply.code(404).send({ error: `profile not found: ${id}` });
        const parsed = runConfigSchema.safeParse(p.config);
        if (!parsed.success) {
          return reply
            .code(400)
            .send({
              error: `profile "${p.name}" is invalid`,
    ...
    for (const e of entries) {
      ...
      if (!cap.available) {
        return reply
          .code(409)
          .send({
            error: `runner unavailable for "${e.profileName}": ${cap.detail}`,
          });
      }
    }
    ...
    if (started.length === 0) {
      return reply
        .code(409)
        .send({ error: failed[0]?.error ?? "no run could be started", failed });
    }
  ...
  app.post("/api/socket/probe", async (req, reply) => {
    const parsed = socketSchema.safeParse(
      (req.body as { config?: unknown })?.config,
    );
    if (!parsed.success)
      return reply.code(400).send({ error: parsed.error.issues });
  ...
  app.get("/api/runs/:id", async (req, reply) => {
    ...
    return {
      ...run,
      samples: store.getSamples(id),
      logs: store.getLogs(id).reverse(),
    };
  });
  ...
  app.post("/api/runs/:id/stop", async (req, reply) => {
    ...
    return ok
      ? { ok: true }
      : reply.code(404).send({ error: "run not active" });
  });
  ...
  app.get("/api/runs/:id/export.csv", async (req, reply) => {
    ...
    const run = store.getRun(id);
    if (!run?.summary)
      return reply.code(404).send({ error: "run has no summary yet" });
    ...
    switch (q.type) {
      case "timeseries":
        body = csv.timeseriesCsv(store.getSamples(id), o);
        break;
      case "checks":
        body = csv.checksCsv(run.summary, o);
        break;
      case "thresholds":
        body = csv.thresholdsCsv(run.summary, o);
        break;
      case "errors":
        body = csv.errorsCsv(run.summary, o);
        break;
      case "custom-metrics":
      ...
      case "all":
        ...
        break;
      default:
        body = csv.summaryCsv([run.summary], o);
    }
    if (body == null)
      return reply.code(404).send({ error: "nothing to export" });

    return sendCsv(
      reply,
      body,
      `loadtest-${slug(run.profileName)}-${q.type ?? "summary"}-${stamp(run.startedAt)}.csv`,
    );
  });
  ...
  app.get("/api/export/runs.csv", async (req, reply) => {
    ...
    return sendCsv(
      reply,
      csv.summaryCsv(summaries, o),
      `loadtest-runs-${stamp(Date.now())}.csv`,
    );
  });
  ...
  app.post("/api/kafka/monitor/start", async (req, reply) => {
    ...
    if (!body?.bootstrapServers)
      return reply.code(400).send({ error: "bootstrapServers required" });
    const interval = Math.min(
      Math.max(
        body.intervalSec ?? store.getSettings().kafkaMonitorIntervalSec,
        1,
      ),
      300,
    );
  ...
  app.post("/api/kafka/monitor/stop", async () => {
    stopMonitor();
    return monitorStatus();
  });
  ...
  app.post("/api/kafka/monitor/auth/import", async (req, reply) => {
    ...
    const content = typeof body?.content === "string" ? body.content : "";
    if (!content.trim())
      return reply.code(400).send({ error: "empty document" });
    if (content.length > 200_000)
      return reply
        .code(413)
        .send({ error: "document too large (limit 200 KB)" });
    try {
      const result = importKafkaAuth(content);
      const parsed = kafkaAuthSchema.safeParse(result.auth);
      if (!parsed.success)
        return reply.code(422).send({ error: parsed.error.issues });
      return result;
    } catch (err) {
      return reply.code(422).send({ error: (err as Error).message });
    }
