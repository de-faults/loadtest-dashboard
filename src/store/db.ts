...
function toRun(r: RunDbRow): RunRow {
  return {
    ...
    endedAt: r.ended_at,
    // Runs persisted before customMetrics existed have no such key in their JSON.
    summary: r.summary
      ? ({
          customMetrics: [],
          ...(JSON.parse(r.summary) as Partial<RunSummary>),
        } as RunSummary)
      : null,
  };
}
...
export function finishRun(
  ...
  db.prepare(
    "UPDATE runs SET state=?, ended_at=?, summary=?, histogram=? WHERE id=?",
  ).run(
    state,
    endedAt,
...
export function insertLog(
  ...
  db.prepare("INSERT INTO run_logs(run_id,ts,level,line) VALUES(?,?,?,?)").run(
    runId,
    ts,
    level,
    line,
...
export function listRuns(limit = 200): RunRow[] {
  return (
    db
      ...
      .all(limit) as unknown as RunDbRow[]
  ).map(toRun);
}

export function getRun(
  id: string,
): (RunRow & { config: RunConfig; histogram: string | null }) | null {
  const r = db.prepare("SELECT * FROM runs WHERE id=?").get(id) as unknown as
    | RunDbRow
    | undefined;
  if (!r) return null;
  return {
    ...toRun(r),
    config: JSON.parse(r.config_snapshot) as RunConfig,
...
export function getSamples(runId: string): WindowMetrics[] {
  ...
  return rows.map((r) => ({
    ...
    vus: r.vus as number,
    latency: {
      min: (r.lat_min ?? 0) as number,
      avg: (r.lat_avg ?? 0) as number,
      p90: (r.lat_p90 ?? 0) as number,
      p95: (r.lat_p95 ?? 0) as number,
      p99: (r.lat_p99 ?? 0) as number,
      max: (r.lat_max ?? 0) as number,
    },
    consumerLag:
...
export function getLogs(
  runId: string,
  limit = 500,
): Array<{ ts: number; level: string; line: string }> {
  return db
    .prepare(
      "SELECT ts,level,line FROM run_logs WHERE run_id=? ORDER BY ts DESC LIMIT ?",
    )
    .all(runId, limit) as unknown as Array<{
    ts: number;
    level: string;
    line: string;
  }>;
